...
## 1. 안드로이드 스튜디오로 IoTivity Client 개발환경 구축
...
## 2. Example App 카피
Light.java 파일과, SimpleClient.java파일을 복사하여 적용한다. 다만 SimpleClient.java파일은 MainActivity.java 파일로 변경되어 적용한다. xml layout 파일도 activity_main.xml 파일로 변경하여 적용하며, 중요한 것은 Manifest 파일에서 퍼미션 관련 설정들을 해주지 않으면, IoTivity Platform Config도중에 에러를 내고 앱이 종료될 것이다.

## 3. 설정 액티비티 추가
설정 액티비티를 추가하기 위해서 안드로이드 스튜디오에서 기본적으로 제공하는 SettingActivity를 추가해본다. 메인화면에서 액티비티 실행을 바인딩 시켜야 하는데, 해당 부분의 구글 API 레퍼런스는 다음과 같다.     
[Activity 띄우기 Reference](https://developer.android.com/training/basics/firstapp/starting-activity.html?hl=ko)    

SettingActivity의 경우 별도의 유저 가이드 문서가 또한 존재한다. 확인하며 개발하면 될 것같다.    
[Setting 액티비티 Reference](https://developer.android.com/guide/topics/ui/settings.html)    
